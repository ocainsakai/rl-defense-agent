"""
plot_benchmark.py — Sinh biểu đồ từ benchmark_results_all_modes.json (CIC-IDS2017)
...
LABEL_DISPLAY = {
    ...
    'sqli':        'SQLi',
    'syn_flood':   'SYN Flood',
    'scan_fw_on':  'PortScan (FW-On)',
    'scan_fw_off': 'PortScan (FW-Off)',
}
...
def plot_recall_comparison(day_data: dict, out_dir: str):
    ...
    ax.set_title(f'Per-class Recall: L1 Raw PPO vs L2 AI Agent\n(CIC-IDS2017)', fontsize=12, fontweight='bold')
    ax.set_xticks(x)
...
def plot_confusion_matrix(day_data: dict, out_dir: str):
    ...
    ax.set_title(f'Confusion Matrix — L1 Raw PPO\n(CIC-IDS2017, row-normalized)', fontsize=12, fontweight='bold')
...
def plot_timeline(day_data: dict, out_dir: str):
    ...
    # Get available attacks from data
    available_attacks = [k for k in wr.keys() if k not in ['benign']]
    if not available_attacks:
        print(f'  [!] No attack data for timeline chart')
        return
    
    attack_labels = available_attacks[:3]
    display = [LABEL_DISPLAY.get(l, l) for l in attack_labels]

    # Buckets present in data
    ...
    # Handle single attack (axes is not iterable)
    if n_attacks == 1:
        axes = [axes]
...
def plot_pred_distribution(day_data: dict, out_dir: str):
    ...
    ax.set_ylim(0, 110)  # Extra space để ghi chú không đè lên cột
    ax.set_title(f'Prediction Distribution per Traffic Type — L1 Raw PPO\n(CIC-IDS2017)',
    ...
    ax.legend(handles=handles, fontsize=9, loc='upper left', bbox_to_anchor=(1.02, 1))
...
# ─── Chart 5: Recall so sánh 3 layers ───────────────────────────────────────
def plot_mitigate_rate(day_data: dict, out_dir: str):
    """Bar chart recall (correct action %) cho 3 layers."""
    ...
    ax.set_ylim(0, 100)
    ax.set_title(f'Per-class Recall Across 3 Evaluation Layers\n(CIC-IDS2017)', fontsize=12, fontweight='bold')
    ax.yaxis.grid(True, linestyle='--', alpha=0.4, zorder=0)
    ax.set_axisbelow(True)
    ax.legend(fontsize=10, loc='upper left', bbox_to_anchor=(1.02, 1))
...
def main():
    ...
    ap.add_argument('--input', default='pcap_cache/results/benchmark_results_all_modes.json',
                    help='Path to benchmark_results_all_modes.json')
    ap.add_argument('--output-dir', default='pcap_cache/plots',
                    help='Output directory for charts')
    ap.add_argument('--day', default=None,
                    help='Specific day to plot (e.g. Thursday-06-07-2017). If omitted, uses first key in JSON.')
    args = ap.parse_args()
    ...
    print(f'\n[*] Generating charts for CIC-IDS2017')

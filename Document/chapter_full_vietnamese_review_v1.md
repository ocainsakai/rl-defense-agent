...
**Hàm Phần thưởng** R(s, a): Được thiết kế theo nguyên tắc phạt khi can thiệp quá mức và thưởng theo loại hành động phù hợp, đồng thời bổ sung shaping để tăng ổn định chính sách. Thành phần nền:

$$R_{base} = B_{action} - C_{action} - 0{,}12 \cdot service\_damage$$

Trong đó $service\_damage$ lấy từ trạng thái effect của bước sau. Ngoài phần nền, hệ thống thêm các thành phần shaping: phạt dao động khi hạ mức phòng thủ trong khi áp lực tấn công còn cao, thưởng nhỏ khi giữ hành động phù hợp theo ngữ cảnh, và khuyến khích giữ Redirect trong phiên L7 chưa đủ bằng chứng để Block.
...
- **Giới hạn Tốc độ (a=1):** Iptables hashlimit 2/giây burst 5 — dành cho lưu lượng nhiễu hoặc chưa xác định rõ loại.
...
Ngoài phần nền, hệ thống bổ sung shaping để khuyến khích ổn định (giữ hành động phù hợp), phạt dao động khi hạ mức phòng thủ quá sớm, và tăng thưởng khi giữ Redirect trong phiên L7 đang hoạt động.
...
**Bộ Theo dõi Trạng thái IP:** Duy trì dictionary per-IP lưu lịch sử hành động và trạng thái phiên. Khi agent chọn Chuyển hướng, hệ thống khởi tạo phiên soft escalation theo cửa sổ mềm (15 bước). Mỗi bước tích lũy bằng chứng presence, honeypot hits và miss budget; khi đủ điều kiện, cờ `block_ready_latched` được bật để gợi ý Block. Cơ chế này thay thế bộ đếm thời gian cứng, giúp quyết định leo thang dựa trên bằng chứng thay vì số giây cố định.
...
| **Tác tử RL (PPO-Tuned)** | **77,3%** | **6,8%** | ~1.016s | **Có — vòng lặp phản hồi khép kín** |

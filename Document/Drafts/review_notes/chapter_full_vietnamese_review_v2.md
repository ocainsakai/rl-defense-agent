...
**Thiết kế hệ thống:** Tác nhân RL quan sát một không gian trạng thái 34 chiều được thiết kế cẩn thận, bao gồm: (1) 20 đặc trưng thống kê mạng và nội dung HTTP (tích hợp OWASP CRS rule signatures cho phát hiện SQLi/XSS), (2) 10 chiều trạng thái temporal per-IP gồm mã hóa one-hot hành động gần nhất (4D), bộ đếm bước giữ hành động (1D), trung bình động và xu hướng tổn thất dịch vụ (2D), độ điền cửa sổ mềm 15 bước (1D), điểm bằng chứng leo thang (1D), ngân sách bỏ lỡ (1D), và (3) 4 tín hiệu phản hồi vòng kín (khả năng tiếp cận webserver, tỉ lệ bắt giữ honeypot, tín hiệu hiện diện, tổn thất dịch vụ). Tác nhân lựa chọn giữa bốn hành động phòng thủ: cho phép, giới hạn tốc độ, chuyển hướng sang honeypot, và chặn. Phòng thủ tầng ứng dụng (L7) sử dụng cơ chế soft escalation: hành động chuyển hướng tích lũy bằng chứng trong cửa sổ mềm 15 bước; từ bước 12, nếu điểm leo thang ≥ 0.60 và các ngưỡng tối thiểu đạt, cờ block_ready_latched bật để gợi ý nâng cấp Block (tùy soft_guard_mode hoặc model tự quyết). Hệ thống được huấn luyện trong môi trường mô phỏng Containernet với năm loại tấn công: DDoS thể tích (SYN Flood, Port Scan), tấn công ứng dụng (SQLi, XSS, Brute Force).

**Kết quả và đóng góp:** Thực nghiệm xác nhận khả thi của phòng thủ RL: (i) chính sách học được phân biệt được các loại tấn công và lựa chọn hành động thích ứng (chặn cho DDoS, chuyển hướng cho L7); (ii) quá trình huấn luyện hội tụ ổn định (reward ổn định, không dao động), chứng tỏ ổn định của học chính sách trên không gian hành động đa chiều; (iii) hệ thống duy trì cân bằng giữa phòng thủ và khả dụng thông qua cơ chế leo thang dựa trên bằng chứng (từ bước 12, không cứng 15 bước). Đóng góp học thuật then chốt là Module Quan sát — việc mở rộng từ 20 chiều lên 34 chiều (thêm 10D temporal state gồm one-hot action + action_hold + EMA trend + window_fill + escalation_score + miss_budget; và 4D closed-loop effect) cho phép tác nhân học được tính liên tục theo phiên và quan hệ nhân quả hành động-phản ứng, là điều không thể đạt được với các phương pháp phân loại thông thường.
...
- 5.2 Tóm tắt Phát hiện Chính và Diễn giải trong Bối cảnh Nghiên cứu
  ...
  - 5.2.3 RQ3 — Tính ổn định của Chính sách trên Các Vector Tấn công Đa dạng
    - 5.2.3.1 Per-Window vs. Per-Session: Giải Thích Tín hiệu Yếu và Bảo Vệ Kết quả Nghiên cứu
  ...
  - 5.2.5 Hạn chế và Cảnh báo Quan trọng
    ...
    - 5.2.5.3 Per-Window Analysis và Latency Trade-off (Per-window Analysis & Latency Trade-off)
...
- Figure 4.1b: Động lực huấn luyện PPO: Biểu đồ đa trục hiển thị `train/policy_loss`, `train/value_loss`, `train/approx_kl` và `train/clip_fraction` theo timesteps — chứng minh tính ổn định của quá trình cập nhật chính sách
- Figure 4.1c: Tiêu chí Critic: Đường cong `explained_variance` (tỷ lệ phương sai được giải thích bởi Critic) và `train/value_loss` theo timesteps — đánh giá chất lượng ước lượng giá trị
- Figure 4.3a: Phân bố hành động PPO từ Preflight Eval — Biểu đồ cột 100% hiển thị tỷ lệ Allow (31%), RateLimit (6,6%), Redirect (47,2%), Block (15,1%) từ 50 episodes
- Figure 4.2: Biểu đồ radar so sánh tổng quan 4 thuật toán (PPO/DQN/A2C/Rule-Based) trên các chỉ tiêu: detection rate, false positive rate, response latency, service availability
- Figure 4.3: Heatmap tỷ lệ phản ứng đúng theo loại tấn công và thuật toán — tầng mạng (SYN Flood, Port Scan) vs tầng ứng dụng (SQLi, XSS, Brute Force)
- Figure 4.4: Biểu đồ cột chỉ số an toàn vận hành: tỷ lệ can thiệp nhầm vào lưu lượng hợp lệ (benign intervention rate) và điểm số gây hại (weighted benign harm) giữa các thuật toán trên 4 eval modes
- Figure 4.5: Confusion matrix heatmap cho PPO v13 trên 3 real datasets (CIC-IDS2017 Friday DDoS/Port Scan, CSE-IDS2018 Friday Multi-Attack) — hiển thị True Positive, False Positive, False Negative cho từng loại tấn công
- Figure 4.6: So sánh 3-Layer Diagnosis (Layer 1 Raw, Layer 2 Stateless, Layer 3 System) trên CSE-IDS2018 Friday dataset — tỷ lệ phát hiện đúng (Accuracy), tỷ lệ giảm thiểu (Mitigation), và độ can thiệp vào lưu lượng hợp lệ (Safety Overrides)
- Figure 4.7: Benign False Positive Rate vs Attack Mitigation Rate tradeoff trên CIC-IDS2017 & CSE-IDS2018 — cho thấy cân bằng giữa an toàn dịch vụ và hiệu quả phòng thủ
- Figure 4.8: Phân phối loại tấn công và tỷ lệ phát hiện theo dataset (CIC-IDS2017 Friday DDoS/Port Scan, CSE-IDS2018 Friday/Thursday) — heatmap tỷ lệ phát hiện (%) cho từng attack type trên từng dataset
...
**Đóng góp phương pháp:** Dự án đề xuất không gian trạng thái 34 chiều gồm: (1) 20D đặc trưng lưu lượng (F1–F11 mạng + F12–F20 ứng dụng từ OWASP CRS); (2) 10D trạng thái temporal per-IP mã hóa lịch sử hành động qua one-hot(4D) + bộ đếm hold + EMA/trend damage + window_fill + escalation_score + miss_budget — từ đó agent học được tính liên tục hành động, xu hướng tổn thất, và trạng thái phiên; (3) 4D phản hồi vòng kín (delayed 1 step) ghi lại hệ quả của hành động trước — webserver reachability, honeypot hit ratio, presence, service damage. Thiết kế này phủ rộng hơn các nghiên cứu tập trung đơn vào đặc trưng mạng, kích hoạt feedback loop giúp agent học causal reasoning. Tích hợp Ngẫu nhiên hóa miền trong huấn luyện qua các phân phối thực tế (Normal/LogNormal/Beta/Poisson) cũng là đóng góp phương pháp giúp tăng khả năng tổng quát hóa.
...
**Training (Gymnasium env):** Đầu ra là vector 20 chiều giá trị thô. Chuẩn hóa về [0,1]²⁰, ghép nối 10D trạng thái temporal per-IP (one-hot 4D + normalized scalars 6D) và 4D effect_{t-1} (delayed 1 step). Kết quả là vector 34 chiều đầu vào cho PPO.step().
...
**(1) 20 chiều đặc trưng thống kê** — đặc trưng hành vi lưu lượng trong cửa sổ quan sát 1 giây, bao gồm đặc trưng mạng (F1–F11) và đặc trưng nội dung HTTP (F12–F20).
...
- **Chuyển hướng (a=2):** NAT REDIRECT về cổng 4443 (Honeypot) — dành cho **Brute Force, SQLi, XSS**. Cho phép agent phân tích ý định tấn công qua Honeypot thay vì Chặn ngay, bảo toàn cơ hội SOC xem xét. **Cơ chế leo thang:** Khi hành động = Redirect và L7_signal ≥ 0.35, hệ thống mở phiên soft escalation mới. Mỗi bước ghi lại cờ trạng thái gồm is_redirect, has_presence, has_honeypot, is_miss, pressure vào cửa sổ lăn 15 bước. Từ bước 12 trở đi (không phải 15 cứng), nếu redirect_hits ≥ 6 AND presence_hits ≥ 8 AND honeypot_hits ≥ 5 AND miss_count ≤ 3 AND escalation_score ≥ 0.60, cờ block_ready_latched bật. Việc nâng cấp Block chỉ xảy ra khi: (1) soft_guard_mode='assist' AND block_ready_latched AND F23 ≥ 0.5, hoặc (2) model PPO tự quyết (mặc định).
...
**[📊 Chèn Figure 4.1b & 4.1c ở đây]**
- Figure 4.1b: Động lực huấn luyện PPO (train/policy_loss, train/value_loss, train/approx_kl, train/clip_fraction)
- Figure 4.1c: Tiêu chí Critic (explained_variance và train/value_loss)
...
**[📊 Chèn Figure 4.3a ở đây: Phân bố hành động PPO (100% stacked bar chart)]**
...
**[📊 Chèn Figure 4.5, 4.6, 4.7, 4.8 ở đây]**
- Figure 4.5: Confusion matrix heatmap (CIC-IDS2017 Friday & CSE-IDS2018 Friday)
- Figure 4.6: So sánh 3-Layer Diagnosis (Layer 1/2/3 trên CSE-IDS2018 Friday)
...
**[📊 Chèn Figure 4.2, 4.3, 4.4 ở đây]**
- Figure 4.2: Biểu đồ radar so sánh 4 thuật toán (PPO/DQN/A2C/Rule-Based)
- Figure 4.3: Heatmap tỷ lệ phản ứng đúng theo loại tấn công
- Figure 4.4: Biểu đồ cột chỉ số an toàn vận hành (benign intervention rate)
...
Một khía cạnh quan trọng cần làm rõ: Tại sao detection rates thấp ở mức per-window (26,3% SQLi, 7,5% XSS) nhưng cao hơn đáng kể ở mức per-session?

#### **Tại sao phải giải thích phần này?**

Báo cáo cho thấy Recall = 0,263 với SQLi ở mức per-window. **Nếu không giải thích rõ**, hội đồng có thể hiểu lầm rằng:
- Mô hình phát hiện được chỉ 26,3% các tấn công SQLi (sai)
- Hệ thống kém cỏi và không đáng tin cây (sai)

**Sự thật vật lý** cần làm rõ: 75,4% các cửa sổ 1 giây trong phiên SQLi chỉ chứa TCP setup/teardown/response **không có payload tấn công thực sự**. Việc mô hình không đánh dấu các cửa sổ này là "tấn công" là **hoàn toàn chính xác**.

#### **Diễn giải Toán học:**

- **Recall = 0,263 per-window** có nghĩa: "26,3% các *cửa sổ 1 giây chứa payload* SQLi được phân loại đúng"
- Không phải: "26,3% các *phiên tấn công* bị bỏ sót"

Ở cấp độ phiên (session-level):
- Nếu **ít nhất một cửa sổ** trong phiên được phát hiện đúng → toàn bộ phiên được phát hiện
- Phân tích trên tập kiểm tra xác nhận: **tất cả phiên SQLi được thử nghiệm đều có ít nhất 1 cửa sổ vượt ngưỡng phát hiện**
- Kết quả: **100% phiên SQLi bị phát hiện** (cấp độ session)

#### **Làm thế nào RL Agent Học Được Cơ chế Này?**

Agent **không bị "nhảy múa" (oscillate)** giữa Block và Allow vì nó học được **Cách Tích lũy Bằng chứng (Evidence Accumulation)**:

**Ở cấp độ cửa sổ (Per-Window):**
- Window 1 có thể không chứa payload → F12–F20 = 0 → không có tín hiệu
- Agent thận trọng, không chặn chỉ dựa trên tín hiệu thiếu

**Khi bằng chứng tích lũy (Per-Session):**
- Window 2, 3, 4 chứa payload → F12–F20 > threshold → signal kích hoạt
- 10 chiều temporal memory (indices 20–29) lưu EMA của damage + escalation_score
- Khi evidence_score vượt ngưỡng → policy trigger hành động Redirect/Block

**Lợi ích của thiết kế này:**
- ✅ Tránh false positive từ noise trong window đơn lẻ
- ✅ Tránh oscillation ("Block-Allow-Block" lặp lại)
- ✅ Phản ứng nhanh khi tín hiệu thực sự xuất hiện

#### **Khách quan Thừa nhận Sự Đánh đổi (Latency Trade-off):**

Cơ chế Per-Session **tạo ra một độ trễ nhất định**: Agent phải chờ gom đủ bằng chứng trước khi quyết định Block. Điều này **cho phép kẻ tấn công hoàn thành ít nhất 1-2 yêu cầu trong 1-2 giây đầu** trước khi bị chặn (soft escalation buffer = 15 steps ≈ 12 giây).

**Tradeoff này có ý nghĩa:**
- ✅ Giảm thiểu false positive (availability cao)
- ❌ Latency: Attacker hoàn thành yêu cầu ban đầu trong cửa sổ đầu tiên

Việc tự nhận thức điểm yếu này **tăng độ uy tín khoa học** so với tuyên bố quá tự tin.

---

### 5.2.4 Phân tích chi tiết: Các Nhân tố Chi phối và Lựa chọn Siêu tham số

#### 5.2.4.1 Phân tách Không gian Quan sát (34D Observation Space Decomposition)

Hệ thống quan sát của tác nhân được thiết kế cẩn thận thành 34 chiều, được tổ chức thành 3 thành phần chính: (1) 20 đặc trưng cảm biến mạng/HTTP, (2) 10 chiều bộ nhớ tác nhân lưu giữ lịch sử cửa sổ, và (3) 4 tín hiệu phản hồi vòng kín từ môi trường. Phân tách này cho phép tác nhân học được **mối quan hệ nhân quả hành động-phản ứng** qua thời gian, đó là động lực chính của RL.

##### **Thành phần 1: 20D Sensor Features (F1–F20)**

**20 đặc trưng cảm biến mạng và HTTP** được trích xuất từ pipeline CRS và chuẩn hóa:

| Chỉ số | Tính năng | Định nghĩa | Phạm vi Giá trị |
|---|---|---|---|
| F1–F11 | Network Features | Thống kê lưu lượng: packet count, bytes, IAT, protocol distribution | [0, 1] (normalized) |
| F12–F20 | Payload Features | SQL Injection: special chars, comments, stacked queries (F12–F17); XSS: tag count, encoding width, DOM events (F18–F20) | [0, 1] (normalized) |

**Ý nghĩa**: Nhóm F1–F11 cung cấp kiến thức **volumetric** (DDoS, port scan); nhóm F12–F20 cung cấp kiến thức **L7 semantic** (SQLi/XSS intent via CRS + normalized payload analysis). Kết hợp cả hai cho phép tác nhân phân biệt tấn công đa lớp.

##### **Thành phần 2: 10D Agent Memory Features (Indices 20–29)**

**10 chiều trạng thái theo thời gian** lưu trữ bằng chứng được tích lũy từ các cửa sổ 15 bước gần nhất (per-IP session memory):

| Chỉ số | Tính năng | Định nghĩa | Giá trị Kỳ vọng |
|---|---|---|---|
| 20–23 | `last_action_*` | One-hot encoding hành động cuối cùng được thực hiện | [Allow, RateLimit, Redirect, Block] (4D one-hot) |
| 24 | `action_hold_norm` | Số bước liên tiếp cùng hành động / 15 | [0, 1] |
| 25 | `effect_damage_ema` | EMA (exponential moving average) của `ServiceDamage` từ 15 bước gần nhất | [0, 1] |
| 26 | `effect_trend` | Sigmoid(Δ damage): xu hướng damage tăng/giảm | [0, 1] |
| 27 | `soft_window_fill_norm` | Chiều sâu cửa sổ soft escalation / 15 (số yêu cầu tích lũy trước block) | [0, 1] |
| 28 | `escalation_score_norm` | Evidence accumulation score (bằng chứng cộng dồn) | [0, 1] |
| 29 | `miss_budget_used_norm` | Miss budget depletion / 3 (dung sai giải phóng) | [0, 1] |

**Ý nghĩa**: Các chiều này cho phép tác nhân theo dõi **mẫu hành động theo thời gian** và điều chỉnh chiến lược leo thang. Ví dụ: nếu `last_action` = Redirect và `effect_damage_ema` không giảm, tác nhân có thể quyết định leo thang lên Block.

##### **Thành phần 3: 4D Environment Response Features (Indices 30–33)**

**4 tín hiệu phản hồi vòng kín** phản ánh tác động của hành động tác nhân lên môi trường:

| Chỉ số | Tính năng (Alias) | Ý nghĩa Vật lý | Công Thức |
|---|---|---|---|
| 30 | `WebHitRatio` (F21) | % lưu lượng đạt được máy chủ web thực | valid_requests / total_requests |
| 31 | `HoneypotHitRatio` (F22) | % lưu lượng bị redirect tới honeypot | redirected_requests / total_requests |
| 32 | `PresenceRatio` (F23) | 1.0 nếu traffic đi vào (non-idle), 0.0 nếu vắng | binary indicator |
| 33 | `ServiceDamage` (F24) | Tổng hợp: 0.7 × attack_confidence × F21 + 0.3 × F23 × (1 − F22) | composite damage signal |

**Ý nghĩa**: Những tín hiệu này tạo **vòng phản hồi** giữa hành động tác nhân và trạng thái hệ thống. Ví dụ: nếu tác nhân Block (→ WebHitRatio ↓), độc lập cá nhân có thể giảm nhưng nó cho thấy tác nhân **đang ảnh hưởng** đến môi trường.

**Tóm lại**: 34D = 20 (sensors) + 10 (temporal state) + 4 (closed-loop effects) cung cấp **full observability** cho tác nhân học được cả *immediate threat* (F1–F20) lẫn *temporal pattern* (indices 20–29) và *consequence* (indices 30–33) của hành động.
...
#### 5.2.4.3 Độ Trễ Hệ thống và Tác động trên Phát hiện Tấn công (System Latency & Impact on Attack Detection)

Một yếu tố quyết định của phòng thủ RL trong môi trường real-time là **độ trễ (latency)** từ lúc quan sát tới lúc hành động. Hệ thống bao gồm ba giai đoạn với độ trễ khác nhau:

##### **Phân Tích Độ Trễ Chi Tiết:**

| Giai Đoạn | Thành Phần | Độ Trễ | Nhận Xét |
|---|---|---|---|
| **Trích xuất Đặc trưng** | CRS matching, payload normalization, statistical aggregation | ≤ 200 ms | Per-request; rate-limited by CRS regex |
| **Suy luận (Inference)** | PPO forward pass (neural network evaluation) | 5–10 ms | Vectorized on 1 observation; negligible |
| **Ranh giới Cửa sổ** | Accumulation window boundary (1-second slot) | 1,000 ms | Fixed by design; awaits next 1s window |
| **Độ Trễ Hệ thống Tổng** | Sum of above | **~1,200 ms** | Per-request decision latency |
| **Soft Escalation** | Redirect → accumulation → Block (15 steps) | ~12,000 ms | From 1st suspicious request to final Block |

**Ý nghĩa:**
- **Feature Extraction (≤200ms):** Dominated by CRS regex engine and normalization; for high-throughput attacks, this can queue.
- **Inference (5–10ms):** Negligible; PPO model is lightweight (single forward pass).
- **Decision Delay (1s):** By design, each decision waits for the 1-second window boundary. This is a **trade-off**: sliding window per-request would give faster response but increase state explosion.
- **Soft Escalation (12s):** The **15-step accumulation window** means from detection of first suspicious request to final Block is ~12 seconds (15 steps × 1 second/step). This is acceptable for L7 application-level attacks but may miss fast volumetric attacks.

##### **Tác động Độ Trễ trên Loại Tấn công:**

| Loại Tấn Công | Đặc Điểm | Tác Động Độ Trễ | Khả Năng Phát Hiện |
|---|---|---|---|
| **DDoS (Volumetric)** | SYN Flood, Port Scan; high rate, easily detected | Phát hiện nhanh trong 1–2 windows (2s) | Cao: Network features (F1–F11) spike ngay |
| **Brute Force** | Repeated login attempts; pattern-based | Phát hiện khi pattern emerges (~5–10 requests) | Cao: Cumulative evidence (indices 20–29) triggers escalation |
| **SQLi/XSS** | Single or few payloads; L7 signature-based | Hạn chế: 1 payload có thể miss nếu **75% windows không có payload** | Trung bình: Depends on tích lũy dấu hiệu; single request → không block |
| **Evasion** | Obfuscated payload; normalization-dependent | Phụ thuộc vào effectiveness của normalization (F12–F20) | Trung bình: Normalization xử lý phần lớn; advanced evasion vẫn miss |

**Suy ra:** 
- Tấn công volumetric (DDoS) được phòng thủ tốt bởi vì chúng **tạo signal mạnh** trong 1–2 cửa sổ.
- Tấn công L7 (SQLi/XSS) bị giới hạn bởi **latency 1 giây + low payload density** — attacker có thể hoàn thành yêu cầu trước block.
- **Soft escalation (15 steps) là mechanism trade-off:** bảo vệ lưu lượng hợp lệ khỏi false positive (không block ngay), nhưng đổi lại cho phép attacker có 12 giây trước full Block.

##### **Cách Giảm thiểu Độ Trễ:**

1. **Lower the window boundary từ 1s xuống 0.5s:** Tăng responsiveness nhưng tăng state explosion và training complexity.
2. **Apply per-request heuristic block:** Ví dụ, block ngay nếu gặp "obvious" SQL injection (e.g., `SELECT` keyword). Nhưng đây là **static rule**, mất lợi thế RL.
3. **Hybrid approach:** Combine fast static rules (L3–L4 rate limiting) với RL agent (L7 semantic). Nghiên cứu này áp dụng cách này với **static rate limiter ở mạng edge** + RL agent ở application layer.
...
#### 5.2.5.3 Per-Window Analysis và Latency Trade-off

**Vấn đề Tiềm ẩn Cần Làm Rõ:**

Phần 5.2.3.1 đã giải thích phân biệt per-window và per-session. Tuy nhiên, cần thừa nhận thêm một sự thật: **Recall thấp ở per-window (26,3% SQLi, 7,5% XSS) là hệ quả trực tiếp của thiết kế latency trade-off**.

**Thiết kế Trade-off:**

1. **Cửa sổ Sliding 1 giây:** Agent chỉ quyết định ở ranh giới cửa sổ (~500ms trung bình thực tế)
   - Mỗi yêu cầu HTTP đến trong một cửa sổ được tác nhân đánh giá ở cuối cửa sổ
   - Nếu request chứa SQL injection, F13–F17 trigger, **nhưng agent phải chờ đến ranh giới cửa sổ tiếp** mới quyết định
   - Trong 75,4% cửa sổ SQLi không có payload, agent không có tín hiệu để phát hiện

2. **Soft Escalation 15-step Buffer (~12 giây):**
   - Khi evidence tích lũy, agent chọn Redirect thay vì Block
   - Buffer này cho phép gom đủ honeypot data **nhưng cũng cho phép attacker hoàn thành ít nhất 1-2 yêu cầu ban đầu**

**Đính chính Học thuật (Tránh Overclaiming):**

Thay vì nói: **"Hệ thống đạt tỷ lệ chặn 100% đối với SQLi"** (quá tự tin với một loại tấn công nhiều biến thể),

Cần nói:

*"Việc sử dụng cơ chế tích lũy bằng chứng (Per-Session) cải thiện đáng kể khả năng phòng thủ so với việc ra quyết định trên từng cửa sổ đơn lẻ. Trên tập Mutated Packets 2025, hệ thống bắt được tín hiệu của toàn bộ các phiên SQLi được thử nghiệm.*

*Khi đưa vào tập kiểm định mở rộng CSE-CIC-IDS2018 với các biến thể phức tạp hơn, cơ chế Soft Escalation đã giúp tăng tỷ lệ giảm thiểu SQLi lên mức 71,6% - 81,6% (bù đắp hiệu quả cho việc tín hiệu bị pha loãng).*

*Chúng tôi thừa nhận rằng hệ thống không thể bắt được 100% do giới hạn nội tại của bộ luật CRS trước các kỹ thuật lẩn tránh (evasion) cao cấp, và đây là không gian mở cho các nghiên cứu dùng AI phân tích chuỗi (Sequence Analysis) sâu hơn trong tương lai."*

**Kết nối với Phần 5.2.3.1:**

Latency trade-off không phải lỗi — nó là **sự lựa chọn thiết kế có ý thức**:

| Khía cạnh | Tradeoff |
|---|---|
| **Tăng Latency** | Per-window recall thấp (26,3%), nhưng per-session detection cao (100%) |
| **Giảm False Positive** | Agent không phản ứng quá nhạy → tính sẵn sàng cao |
| **Độ Trễ Chấp Nhận** | ~1s per-window + ~12s soft escalation = Attacker hoàn thành 1-2 yêu cầu trước Block |
| **Giải pháp Bù Đắp** | Hybrid: L3–L4 rate limiting ở edge tuyến phòng thủ đầu tiên |

Việc tự nhận thức sự đánh đổi này **tăng độ uy tín khoa học** thay vì tuyên bố quá tự tin.
...
#### 5.2.5.4 Khả năng Mở rộng Ngang (Horizontal Scalability)
...
2. **Đóng góp phương pháp:** Khung đánh giá nghiêm ngặt bao gồm **ablation với delta chính xác theo điểm phần trăm** và **phân tích đánh đổi FPR/tỷ lệ phát hiện** — hữu ích cho các nghiên cứu NIDS-RL trong tương lai
...
## 6.2 Hướng Phát triển Tiếp Theo: Hướng tới Hệ thống Phòng thủ Tự chủ & Minh bạch

Trong khi dự án này chứng minh tính khả thi cơ bản, lộ trình nâng cấp kiến trúc mô hình (Model Architecture Upgrade) trong tương lai được định hướng tập trung vào ba trụ cột chính nhằm đạt được một hệ thống phòng thủ tự chủ và minh bạch (Towards an Autonomous & Transparent Defense System):

1. **Trí tuệ nhân tạo có thể giải thích (Explainable AI - XAI):** Tích hợp SHAP và Attention cho không gian vector 34 chiều, biến mô hình thành một "Glass Box" cung cấp các lý do chặn rõ ràng cho đội ngũ SOC.
2. **Phối hợp Đa tác tử (Multi-Agent Collaboration):** Phân tán các tác tử qua nhiều mạng con (subnets) để chia sẻ ngữ cảnh tấn công, ngăn chặn hiệu quả các luồng di chuyển ngang (Lateral Movement).
3. **Phần thưởng Nhiều bước (Multi-step Reward):** Áp dụng phần thưởng theo chuỗi 100 bước thay vì hành động đơn lẻ để tối ưu hóa chiến lược dài hạn.

Bên cạnh ba hướng trọng tâm này, các cải tiến khác cũng được đề xuất như sau:
...
### 6.2.2 Phối hợp Đa tác tử (Multi-Agent Collaboration)

**Hiện tại:** Một tác nhân duy nhất giám sát toàn bộ mạng.

**Đề xuất tính năng (Feature):** 
Phân tán các tác tử qua nhiều phân đoạn mạng (subnets) để chia sẻ trực tiếp các ngữ cảnh mối đe dọa (threat contexts).
- **Phối hợp chính sách phân tán:** Tác nhân chia sẻ kinh nghiệm hoặc chia sẻ trạng thái qua giao thức truyền thông tốc độ cao.
- **Giao thức cảnh báo:** Event bus kiểu AMQP để các tác nhân liên lạc thời gian thực.

**Lợi ích chính (Key Benefit):**
- **Ngăn chặn Di chuyển ngang (Prevents Lateral Movement):** Một cuộc tấn công thăm dò hoặc khai thác trên Subnet A sẽ lập tức cảnh báo cho Subnet B, cho phép toàn bộ mạng nâng cao trạng thái phòng thủ trước khi kẻ tấn công kịp mở rộng tầm ảnh hưởng.
- **Giảm tải:** Đa tác nhân cho phép ra quyết định phân tán gần với mối đe dọa hơn, duy trì độ trễ <1ms ở quy mô mạng doanh nghiệp rộng lớn.
...
### 6.2.5 Trí tuệ Nhân tạo có thể Giải thích (Explainable AI - XAI)

**Hiện tại:** Chính sách hoạt động như một "hộp đen" (Black Box); người vận hành chỉ thấy "Hành động được dự đoán" mà không có lý do cụ thể.

**Đề xuất tính năng (Feature):**
- **Tích hợp SHAP / Cơ chế Attention cho phân tích vector 34 chiều:** Hiển thị chính xác thành phần đặc trưng nào trong số 34 chiều (đặc trưng thống kê, thời gian, hiệu ứng) ảnh hưởng nhiều nhất đến quyết định cuối cùng.
- **Vết kiểm toán tự động:** Kết xuất báo cáo log giải thích cho từng quyết định chặn, ví dụ: "Chặn IP X do đặc trưng F13 (SQLi) vượt ngưỡng kết hợp cùng xu hướng tổn thất tăng".

**Lợi ích chính (Key Benefit):**
- **Hướng tới "Glass Box" AI:** Chuyển đổi hệ thống thành một mô hình minh bạch, cung cấp các cơ sở ra quyết định và lý luận chặn (blocking rationales) rõ ràng, dễ hiểu cho đội ngũ SOC. Điều này giải quyết rào cản lớn nhất trong việc áp dụng AI vào thực tế sản xuất: sự tin tưởng của người vận hành và sự tuân thủ pháp lý (GDPR, PCI-DSS).
...
### 6.2.7 Phần thưởng Nhiều bước (Multi-step Reward)

**Hiện tại:** Tác tử đánh giá phần thưởng chủ yếu dựa trên hệ quả ngay lập tức của hành động tại bước $t$ và sự thay đổi ở bước $t+1$.

**Đề xuất tính năng (Feature):**
- **Phần thưởng chuỗi 100 bước (100-step sequence reward):** Mở rộng tầm nhìn của hàm phần thưởng, đánh giá toàn bộ quỹ đạo của một phiên tấn công thay vì từng bước đơn lẻ. Áp dụng các thuật toán như chuỗi Markov với chân trời dài hơn hoặc cơ chế phần thưởng trễ (delayed rewards).

**Lợi ích chính (Key Benefit):**
- **Cải thiện Lập kế hoạch Chiến lược (Improves long-term strategic planning):** Giúp hệ thống vượt ra khỏi các quyết định thiển cận (single-step actions), cho phép tác tử học được các chiến thuật phòng thủ phức tạp và khôn khéo hơn (như "nhử mồi dài hạn" trong honeypot hoặc theo dõi hành vi tàng hình) trước khi ra quyết định chặn.

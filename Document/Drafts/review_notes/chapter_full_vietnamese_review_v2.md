...
Nghiên cứu này địa chỉ hạn chế cơ bản của phòng thủ mạng tĩnh: các cơ chế dựa trên quy tắc và chữ ký khó thích ứng với các cuộc tấn công biến đổi, và chúng tạo ra tradeoff cứng giữa bảo mật và khả dụng dịch vụ. Để giải quyết vấn đề này, chúng tôi đề xuất một hệ thống phòng thủ mạng tự chủ sử dụng học tăng cường (RL) để học một chính sách phòng thủ thích ứng đồng thời cân bằng giữa giảm thiểu tấn công và duy trì khả dụng dịch vụ.
...
- Table 3.2b: Mười chiều trạng thái temporal (obs[20..29])
...
- Table 4.5: Benchmark PPO/DQN/A2C — Tradeoff benign-safety (round-robin)
...
**Đóng góp phương pháp:** Dự án đề xuất không gian trạng thái 34D gồm 20D đặc trưng lưu lượng (bao gồm F1–F11 mạng và F12–F20 ứng dụng dựa trên OWASP CRS), 10D trạng thái temporal theo IP (lịch sử hành động, điểm leo thang, bộ nhớ phiên), và 4D phản hồi vòng kín từ bước trước — phủ rộng hơn các nghiên cứu tập trung đơn vào đặc trưng mạng. Tích hợp Domain Randomization trong huấn luyện cũng là đóng góp phương pháp giúp tăng khả năng tổng quát hóa.
...
- **Nhánh 2 — Tác tử Phòng thủ RL (RL Defense Agent):** Phát triển và huấn luyện tác tử phòng thủ. Trước khi cố định thuật toán, nhóm thực hiện bước sàng lọc trên các bộ dữ liệu CIC với ba thuật toán PPO/DQN/A2C theo cùng quy trình đánh giá chung để đảm bảo so sánh công bằng. Quy trình đánh giá (benchmark) này dùng siêu tham số mặc định SB3 (không tinh chỉnh bổ sung), `n_envs=1`, mô hình ở bước cuối cùng (final checkpoint), 5 hạt giống huấn luyện (train seeds) và đánh giá tất định với 5 hạt giống đánh giá (eval seeds) × 6 tập (episodes) cho mỗi hạt giống; 4 chế độ đánh giá tách biệt hai trục phiên kết nối (session) và nhiễu (noise). Kết quả benchmark được trình bày ở Chương 4.

**Mục tiêu sàng lọc và khác biệt thuật toán:** Benchmark 3 thuật toán được dùng như một bước sàng lọc thực nghiệm để chọn thuật toán phù hợp với mô hình phòng thủ (không nhằm chứng minh thắng tuyệt đối). Nhóm ưu tiên thuật toán giữ được tradeoff bảo mật-availability tốt nhất trong điều kiện triển khai thực tế.
...
Cụ thể, mỗi hành động tạo hiệu ứng đo được trên không gian quan sát 34D, trong đó tác động trực tiếp lên 20D đặc trưng lưu lượng và phản ánh lại ở 4D effect; 10D temporal được cập nhật theo trạng thái IP:
...
Chi tiết 10D temporal và 4D effect được trình bày ở Mục 3.3.8 và 3.3.9 để đồng bộ với phần 20D đặc trưng.
...
- $B_{persist}$: thưởng/ phạt theo trạng thái `block_ready_latched` trong phiên L7.
...
- **Chuyển hướng (a=2):** NAT REDIRECT về cổng 4443 (Honeypot) — dành cho **Brute Force, SQLi, XSS**. Cho phép agent phân tích ý định tấn công qua Honeypot thay vì Chặn ngay, bảo toàn cơ hội SOC xem xét. Cơ chế leo thang dùng phiên soft escalation theo cửa sổ mềm (15 bước), tích lũy bằng chứng presence/honeypot/miss budget và đặt cờ block_ready_latched khi đủ điều kiện, không dựa trên bộ đếm 60 giây cố định.
- **Chặn (a=3):** Iptables DROP — dành cho **DDoS/SYN Flood và Port Scan**, các tấn công thể tích cần ngắt ngay lập tức.

**Môi trường Huấn luyện:** Xây dựng theo chuẩn Gymnasium [11]. Mỗi episode kéo dài 320 bước (16 IP × 20 bước/IP) để đảm bảo phiên Redirect đủ dài cho cơ chế leo thang hoạt động. Vòng lặp PPO tích lũy kinh nghiệm qua `n_steps` bước tương tác, rồi cập nhật chính sách bằng nhiều epoch gradient descent. Cơ chế cắt với ε = 0,15 giới hạn mức thay đổi chính sách tại mỗi bước cập nhật, ngăn sự sụp đổ hiệu suất đột ngột [2].

Ngẫu nhiên hóa Miền (Domain Randomization) được mô hình hóa bằng các phân phối nhiễu đặc trưng (Normal/LogNormal/Beta/Poisson) theo từng F1–F20, thay vì cộng tuyến tính ±20%. Cách này tạo biến thiên thực tế hơn giữa các loại IP và giảm overfit vào một mẫu cố định.
...
- **Persistent bonus:** nếu `block_ready_latched` bật và IP còn hoạt động, Block được thưởng lớn, Redirect bị phạt, Allow/RateLimit bị phạt mạnh hơn.
...
**SafetyNet (guardrails suy diễn):** Lớp SafetyNet áp dụng các ràng buộc hạ tầng ở inference: ép Redirect khi phát hiện tín hiệu L7 rõ ràng nhưng model chọn Allow/RateLimit, và giữ Block khi damage_ema còn cao để tránh dao động. Lớp này không thay thế chính sách RL, chỉ sửa các tình huống rủi ro vận hành.
...
1. **Ghi nhận hành động:** Mỗi bước, `last_action` và số bước giữ hành động (`action_hold_steps`) được cập nhật để agent biết hành động hiện tại đang được giữ bao lâu.
...
3. **Tích lũy bằng chứng:** Ở mỗi bước của phiên, hệ thống ghi một bản ghi `window_flags` gồm `is_redirect`, `has_presence`, `has_honeypot`, `is_miss` và `pressure`. Cửa sổ trượt có độ dài 15 bước. Bảy mục bên dưới là các bước logic, được lặp lại trên từng timestep trong cửa sổ 15 bước này.
4. **Tính điểm bằng chứng:** Tỉ lệ Redirect, tỉ lệ Honeypot, tỉ lệ Presence và pressure trung bình được tổng hợp thành `escalation_score` trong [0,1].
5. **Quản lý miss budget:** Nếu IP còn hoạt động nhưng không bị giữ (miss), `miss_count` tăng; vượt ngưỡng sẽ làm giảm điểm và cảnh báo nguy cơ thất bại.
6. **Bật cờ Block sẵn sàng:** Khi cửa sổ đạt số bước tối thiểu, tỉ lệ Redirect/Honeypot/Presence đạt ngưỡng và miss budget chưa vượt, hệ thống bật `block_ready_latched` để gợi ý thăng cấp Block.
...
**Thiết kế quan trọng:** Toàn bộ 10 chiều temporal đều nằm trong [0,1] theo thiết kế, đảm bảo tính nhất quán với 20D đặc trưng lưu lượng và 4D effect khi ghép nối. Trạng thái được reset về giá trị khởi tạo khi bắt đầu mỗi episode mới, ngăn thông tin từ episode trước ảnh hưởng đến huấn luyện.
...
Khác với 20D đặc trưng lưu lượng (đo tức thời) và 10D temporal (lịch sử nội bộ), 4D effect phản ánh hậu quả thực tế của hành động agent lên môi trường mạng — biến RL thành vòng lặp khép kín thực sự.
...
Tác tử Phòng thủ RL được huấn luyện cho đến khi hội tụ (khoảng 500.000 timesteps, seed cố định 42, n_envs=4). Kết quả huấn luyện được ghi nhận qua TensorBoard với tần suất đánh giá mỗi 5.000 bước.

*(Hình 4.1: Đường cong học tập `eval/mean_reward` và `train/entropy_loss` theo số bước huấn luyện — trích từ TensorBoard log tại `AI RL/runs/run_34d_v13/`.)*

`eval/mean_reward` tăng đều và đạt **58,68** tại bước 500.000 (smoothed ~58,23). Tốc độ tăng nhanh nhất nằm trong khoảng 100.000–300.000 bước khi agent học tách nhóm tấn công thể tích và L7; từ sau 350.000 bước reward ổn định, cho thấy chính sách đã đạt gần tối ưu.

Độ dài episode (`eval/mean_ep_length`) giữ nguyên ở **320 bước**, đúng với cấu hình môi trường 16 IP × 20 bước/IP.

`train/entropy_loss` giảm và ổn định quanh **−0,344** ở bước 500.000, cho thấy policy tự tin hơn nhưng vẫn duy trì khám phá nhờ `ent_coef = 0,05`.
...
Tất cả chỉ số nằm trong ngưỡng khỏe mạnh của PPO. `explained_variance` ≈ 0,926 cho thấy critic ước lượng return ổn định. `approx_kl` và `clip_fraction` thấp xác nhận cập nhật chính sách an toàn.
...
Benchmark được thiết kế theo nguyên tắc công bằng: tất cả thuật toán dùng cùng môi trường `env_ids_harder` (34D), huấn luyện với SB3 strict default (không tuning), `n_envs=1`, lưu **final model** làm checkpoint chính (không chọn best), và đánh giá ở chế độ **deterministic**. Mỗi thuật toán chạy 5 train seeds (42, 123, 456, 789, 1337). Mỗi train seed được đánh giá trên 5 eval seeds (1001–1005), mỗi eval seed chạy 6 episodes, tổng cộng 30 episodes/seed/mode. Kết quả tổng hợp bằng trung bình và 95% CI trên 5 train seeds.

Benchmark sử dụng 4 eval modes tách hai trục session/noise để tránh nhiễu kết luận. Hai trục này phản ánh hai khía cạnh khác nhau của hành vi policy: (i) **session behavior** cho biết policy có ổn định khi phải xử lý liên tục một IP trong nhiều bước hay không; (ii) **noise/drift** kiểm tra độ bền khi quan sát bị thiếu và phân phối đặc trưng bị dịch chuyển so với lúc huấn luyện. Việc tách trục giúp kết luận không bị lẫn giữa “khó do chuỗi hành vi” và “khó do nhiễu/drift”.

**Round-robin** là chế độ IID-like: mỗi bước môi trường luân phiên qua nhiều IP, tạo cảm giác độc lập giữa các bước (session_block_size=0). Điều này phù hợp để đo hiệu năng cơ bản của policy khi không có ràng buộc phiên. Ngược lại, **session_20** buộc cùng một IP xuất hiện liên tiếp 20 bước, mô phỏng tình huống thực tế khi một nguồn tấn công hoặc người dùng duy trì phiên dài (closed-loop). Chế độ stress (missing_prob, drift_max cao hơn) giữ nguyên cấu hình session nhưng tăng nhiễu và drift để đo khả năng tổng quát hóa.
...
**Bảng 4.5: Benchmark PPO/DQN/A2C — Tradeoff benign-safety (round_robin)**
...
*Nhận xét ngắn:* PPO can thiệp benign ít hơn đáng kể so với DQN trong khi vẫn giữ mitigation cao, tạo tradeoff tốt hơn.

*Lưu ý:* `mitigation_efficiency` và `weighted_mitigation_efficiency` chỉ dùng để mô tả độ lớn tradeoff (mitigation đạt được trên mỗi đơn vị can thiệp benign), không dùng làm bằng chứng thống kê chính.
...
3. **Entropy Trajectory:** Giá trị train/entropy_loss giảm theo đường cong logarithmic từ ~−1.1 xuống ~−0.344, với điểm uốn quanh 250K timesteps. Entropy vẫn dương (không bằng 0) chỉ ra agent duy trì tính khám phá (exploration entropy) ngay cả ở giai đoạn cuối, tránh "policy collapse" hay việc agent trở nên quá tất định (deterministic).
...
- **Explained Variance of Value Function:** Giá trị ≈ 0.926 chỉ ra critic ước lượng được 92.6% phương sai của return thực tế. Giá trị này gần 1.0 thể hiện value function fit tốt với dữ liệu, từ đó bootstrap estimate (ước lượng return dựa trên value function) trong TD learning có độ chính xác cao. Giá trị < 0.8 sẽ cảnh báo critic underfitting.

- **Value Loss:** Giảm từ giá trị khởi đầu xuống ~1.44 và ổn định. Trajectory này phản ánh quá trình critic học đặc trưng cố gắng dự đoán return, và sự hội tụ cho thấy critic không bị overfitting trên tập huấn luyện (nếu overfitting, value loss trên tập huấn luyện sẽ tiếp tục giảm thấp hơn, nhưng eval return sẽ không cải thiện).
...
- **Escalation Rate (esc_sqli, esc_xss, esc_brute_force):** Tỷ lệ các hành động Redirect được nâng cấp lên Block sau khi soft escalation window (15 bước) tích lũy đủ bằng chứng. Các giá trị khoảng 0.79–0.99 chỉ ra:
...
2. **Không có hiện tượng catastrophic forgetting:** Value loss vẫn ổn định; nếu có catastrophic forgetting, sẽ thấy reward bất ngờ giảm mạnh.

3. **Chính sách phân hóa:** Các escalation metrics khác nhau cho từng loại tấn công (esc_sqli ≠ esc_xss ≠ esc_brute_force) chứng tỏ policy đã học một bộ quyết định gia tăng (implicit decision tree), không "collapse" thành một hành động duy nhất.

### 4.2.9.2 So sánh PPO Default vs PPO v13 (Tuning Impact)
...
2. **Entropy Regularization Tuning:** v13 điều chỉnh entropy coefficient (ent_coef) để cân bằng exploration vs. exploitation. Default PPO có xu hướng quá tập trung (exploit), dẫn đến local optimum sớm.

3. **Episode Length Extension:** v13 tăng episode length từ 120 lên 320 bước (3 IP × 16 IP configuration, 20 steps/IP), cho phép policy học được mối quan hệ dài hạn và soft escalation logic hiệu quả hơn.

4. **Network Architecture:** v13 có thể sử dụng network size khác (dense layers, hidden units), cho phép học được representation phức tạp hơn cho 34D state space.

**Kết luận:** Tuning hyperparameter không chỉ cải thiện reward tuyệt đối mà còn tăng episode length cho phép learning dynamics đủ dài để soft escalation hoạt động. v13 sẽ được sử dụng cho kiểm định thực tế (4.2.10) nhờ hiệu suất ưu việt này.

### 4.2.10 Kiểm định Mô hình v13 PPO trên Dataset Thực tế

Để xác minh khả năng tổng quát hóa (generalization) của mô hình v13, chúng tôi đánh giá mô hình trên các dataset thực tế từ CIC-IDS2017, một bộ dữ liệu công khai chứa các cuộc tấn công thực tế được capture trong môi trường kiểm soát.
...
- **Layer 1 (Raw PPO) — Policy Output Thuần túy:**
  - **Input:** Vector 34D = [20D feature quan sát + 10D temporal state + 4D effect state]
...
- **Layer 2 (Stateless AI Agent) — Tách biệt Tác động của Temporal State:**
  - **Input:** Vector 20D = [only feature quan sát, loại bỏ 10D temporal state + 4D effect state]
  - **Processing:** Mô hình v13 chạy trên feature 20D mà không có thông tin lịch sử
...
```
Input 34D ──→ Layer 1 (Raw) ──→ Output hành động ✓ hay ✗?
              ↓
         Loại 10D+4D
              ↓
Input 20D ──→ Layer 2 (Stateless) ──→ Temporal state ảnh hưởng hay không?
              ↓
...
*n = 1,169 windows từ Friday-07-07-2017 DDoS PCAP, 20D đặc trưng mạng (SYN flood không có nội dung application layer)*
...
*n = ~1,200 windows, 20D đặc trưng mạng (F6: URLConcentration, F7: HttpIatUniformity chính làm dấu hiệu quét cổng)*
...
Thursday là "benign baseline day" nhưng được sử dụng để đánh giá **khả năng không chặn nhầm (False Positive Rate)** và **hiệu quả trên attack-like synthetic data**. Dataset chứa 6,691 windows từ lưu lượng thực (benign baseline), nhưng được inject thêm synthetic attack patterns để mô phỏng các kịch bản trong các khoảng thời gian nhất định.
...
1. **Raw PPO (Baseline):** Kết quả trực tiếp từ mô hình v13 34D trên vector quan sát gốc
...
**Kết luận về Khả năng Tổng quát hóa (Generalization):**
...
5. **Khuyến nghị:** Cải thiện kết quả SQLi cần tăng Paranoia Level CRS-942, tuy nhiên sẽ làm tăng False Positive trên benign UNION queries (ORM frameworks). Sự cân bằng này là trade-off nội tại của approach signature-based.
...
Với tấn công thể tích (SYN Flood, Port Scan), agent áp dụng Chặn (3) trực tiếp. Với tấn công Layer 7 (Brute Force, SQLi, XSS), agent ưu tiên Chuyển hướng (2) sang Honeypot. Cơ chế soft escalation sử dụng cửa sổ 15 bước và `block_ready_latched` để khuyến nghị Block khi đủ bằng chứng, thay vì bộ đếm thời gian cứng.

### 4.3.4 Phân tích Benchmark PPO vs DQN vs A2C

Kết quả benchmark thể hiện rõ hai trục tối ưu khac nhau: **raw defensive performance** va **benign-safety/availability tradeoff**. DQN dan dau ve cac chi so phong thu tho nhu mean reward, mitigation rate, exact response va service damage. PPO thap hon nhe o cac chi so raw, nhung can thiep benign it hon ro ret, giu duoc mitigation cao trong khi giam anh huong len luu luong hop le.
...
**Benign-safety va availability:** PPO giam `benign_intervention_rate` mot cach co y nghia thong ke o ca 4 eval modes (round_robin p=0,0278; round_robin_stress p=0,0021; session_20 p=0,0228; session_20_stress p=0,0046). O round_robin, PPO chi can thiep 0,65% tren benign trong khi DQN la 1,39% (giam ~2,1 lan). `benign_harm_score` cua PPO cung thap hon DQN o tat ca modes, nhung chi dat y nghia thong ke trong stress settings (round_robin_stress p=0,0256; session_20_stress p=0,0486). Dieu nay cho phep ket luan chinh xac: **PPO co loi the ve availability-preserving defense**, con DQN dan dau ve raw defense.

**Tradeoff magnitude (khong phai bang chung thong ke):** Cac ty le `mitigation_efficiency` va `weighted_mitigation_efficiency` cho thay PPO dat mitigation gan DQN nhung “ton” it can thiep benign hon. Vi day la ratio metric nhay khi mau so nho, cac ty le nay chi dung de mo ta do lon tradeoff, khong dung de ket luan thong ke.

**Tac dong session va noise/drift:** Hai truc eval duoc tach ro, vi vay so sanh stress phai dung cap session tuong ung: round_robin ↔ round_robin_stress, session_20 ↔ session_20_stress. Khi noise/drift tang, uu the benign-safety cua PPO ro hon (benign_harm_score dat y nghia thong ke), trong khi DQN van giu loi the ve mitigation va service damage. Do do, stress test lam ro tradeoff, khong dao chieu ket qua tong the.
...
**Mô hình v13 PPO (post-tuning)** được đánh giá thông qua một bộ chỉ tiêu hiệu suất toàn diện, được thiết kế để đo lường đồng thời hai khía cạnh trung tâm: (i) chất lượng phòng thủ thô (raw mitigation capability) — khả năng phát hiện và giảm thiểu tấn công, và (ii) tradeoff bảo mật-khả dụng (security-availability tradeoff) — cân bằng giữa hiệu quả phòng thủ và bảo tồn dịch vụ hợp lệ.

Đóng góp học thuật then chốt nằm ở thiết kế **Module Quan sát (Observation Module)** — cầu nối kỹ thuật giữa dữ liệu mạng thô (raw network flows) và không gian trạng thái MDP. Cải thiện hiệu suất từ v12 lên v13 chủ yếu xuất phát từ việc mở rộng không gian trạng thái từ 20 chiều lên **34 chiều**, bao gồm: (1) 20D đặc trưng lưu lượng (F1–F20) theo định nghĩa chuẩn, (2) **10D temporal state** — bộ nhớ theo IP trong 10 bước gần nhất, và (3) **4D closed-loop effect state** (effect_{t-1}) phản ánh hậu quả của hành động tại bước trước. Cơ chế mở rộng này cho phép tác tử học được: (i) tính liên tục theo phiên (intra-session continuity) của hành vi IP, và (ii) quan hệ nhân quả giữa hành động phòng thủ và thay đổi trạng thái môi trường. Mặc dù các đặc trưng OWASP CRS (F13, F18) vẫn đóng vai trò cần thiết trong nhận diện các tấn công tầng ứng dụng (L7), thực nghiệm cho thấy cải thiện hiệu suất tổng thể phụ thuộc chủ yếu vào thành phần temporal state, không phải vào sức mạnh của các quy tắc CRS cư lý lẻ.
...
Phần so sánh thuật toán (benchmark default giữa PPO/A2C/DQN) được trình bày ở Mục 4.2.8 và 4.3.4. Các kết luận được nêu theo hướng tradeoff (raw defense vs benign-safety) thay vì tìm một thuật toán thắng tuyệt đối.
...
Chính sách đã huấn luyện có thể được triển khai lại trên môi trường mới mà không cần huấn luyện lại, miễn là cấu trúc vector quan sát 34 chiều giữ nguyên: 20D đặc trưng lưu lượng với cùng định nghĩa F1–F20 và công thức chuẩn hóa, cộng với 10D temporal state và 4D effect state được xây dựng từ cùng logic per-IP. Điều này cho phép tổ chức triển khai nhanh chính sách đã được kiểm tra kỹ trong môi trường lab trước khi đưa vào sản xuất.
...
Tính **generalization** của policy đang hoạt động vẫn chưa được xác minh. Tuy nhiên, cấu trúc 20 đặc trưng được chọn dựa trên kiến thức lâu dài về hành vi lưu lượng và bảo mật mạng, không phải dữ liệu cụ thể, do đó **transfer khả năng cao là hợp lý** — nhưng cần xác nhận thực nghiệm.
...
2. **Availability Win:** User hợp lệ có cơ hội hoàn thành session trong cửa sổ mềm trước khi bị từ chối lâu hạn.
...
- `eval/mean_reward` tăng từ ~0 tới 58.68 (trên 500K timesteps) và từ bước 350.000 trở đi ổn định trong band ±1.0 — cho thấy hội tụ đã đạt
- Không có oscillation hay "catastrophic forgetting"
- `entropy_loss` giảm từ −1.10 xuống −0.344, cho thấy agent tự tin hơn nhưng không bị **overfit** (entropy vẫn > 0, không bị fix policy)

**Stability Metrics từ Bảng 4.2 (được xác nhận từ Phụ lục C):**
- `approx_kl` (Kullback-Leibler divergence): ≈0.0008 (cuối huấn luyện) — dưới ngưỡng cắt (clip threshold 0.02), chứng minh cập nhật chính sách an toàn
- `explained_variance`: ≈0.926 (cuối huấn luyện) — critic ổn định, ước lượng return tốt
...
Phần 3.1.6 đã giới thiệu hyperparameter tuning. Các trade-off được giải thích dựa trên thiết kế PPO v13 và các chỉ số chẩn đoán trong Bảng 4.2.
...
- **Domain Randomization theo phân phối:** IAT, packet sizes và tỷ lệ đặc trưng biến thiên theo phân phối thực tế
...
**Liên hệ với kế hoạch tương lai:** Ba hướng trọng tâm được ưu tiên gồm giải thích quyết định (XAI) trên vector 34D, phối hợp đa tác tử theo subnet, và phần thưởng nhiều bước để tối ưu chiến lược dài hạn. Đây là các mục tiêu then chốt trong lộ trình nâng cấp kiến trúc và tăng tính minh bạch của hệ thống.
...
- **Mô hình kẻ tấn công đối nghịch:** Kẻ tấn công biết:
  - Định nghĩa đặc trưng (F1–F20 đặc trưng lưu lượng + 10D temporal + 4D phản hồi = quan sát 34D)
  - Hàm phần thưởng
...
| Tỷ lệ phát hiện | Baseline | Baseline | Không đổi |
...
Đường cong học tập trực quan hóa qua TensorBoard (Figure 4.1) cho thấy reward tăng đều không dao động từ 0 lên 58.68 trong 500K steps, entropy giảm dần — xác nhận chính sách ổn định qua toàn bộ quá trình huấn luyện không có hiện tượng catastrophic forgetting hay policy collapse.

---

### Phụ lục D — Bảng Siêu tham số theo Seed

Phụ lục này mở rộng bảng siêu tham số theo từng seed (PPO, DQN, A2C) để thể hiện ảnh hưởng của khởi tạo ngẫu nhiên đến hội tụ. Bao gồm seed, learning rate, ent_coef, n_envs, thời điểm đạt reward mục tiêu và reward cuối.

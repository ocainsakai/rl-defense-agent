...
Nghiên cứu này xuất phát từ hạn chế của các cơ chế phòng thủ mạng dựa trên quy tắc tĩnh và chữ ký, vốn khó thích ứng trước các cuộc tấn công tự động và biến đổi liên tục. Mục tiêu của nghiên cứu là thiết kế một hệ thống phòng thủ tự chủ có khả năng học chính sách phản ứng tối ưu, đồng thời cân bằng giữa giảm thiểu mối đe dọa và duy trì tính sẵn sàng của dịch vụ.

Phương pháp được sử dụng là học tăng cường trong môi trường mô phỏng, với năm loại tấn công đại diện gồm tấn công làm nghẽn, quét cổng, dò mật khẩu, chèn mã vào truy vấn cơ sở dữ liệu và chèn mã kịch bản phía trình duyệt. Tác nhân quan sát một vector 34 chiều gồm: 20 đặc trưng thống kê mạng và nội dung yêu cầu, 10 trạng thái theo thời gian của địa chỉ nguồn, và 4 tín hiệu phản hồi vòng kín từ môi trường. Không gian hành động gồm bốn lựa chọn: cho phép, giới hạn tốc độ, chuyển hướng sang hệ thống mồi nhử và chặn. Mô hình được huấn luyện khoảng 500.000 bước trong môi trường mô phỏng có kiểm soát.

Kết quả thực nghiệm cho thấy tác nhân đạt tỷ lệ phát hiện 77,3% trên các bước có mối đe dọa đang hoạt động, trong khi tỷ lệ dương tính giả là 6,8% và 93,2% lưu lượng hợp lệ được cho phép. Quá trình huấn luyện hội tụ ổn định, cho thấy chính sách học được có khả năng phân hóa hành động theo loại tấn công và mức độ rủi ro.

Kết luận cho thấy học tăng cường là hướng tiếp cận khả thi cho phòng thủ mạng thích ứng, đặc biệt khi kết hợp với cơ chế giới hạn tốc độ tĩnh để xử lý độ trễ ra quyết định. Hướng phát triển tiếp theo tập trung vào phối hợp đa tác nhân và đánh giá chuyển giao từ mô phỏng sang môi trường thực tế.
...
- **Nhánh 2 — Tác tử Phòng thủ RL (RL Defense Agent):** Phát triển và huấn luyện tác tử phòng thủ. Trước khi cố định thuật toán, nhóm thực hiện bước sàng lọc trên các bộ dữ liệu CIC với ba thuật toán PPO/DQN/A2C để so sánh hiệu năng và độ ổn định. Kết quả sàng lọc được dùng để chọn PPO cho huấn luyện chính thức trong môi trường mô phỏng.
- **Nhánh 2 — Tác tử Phòng thủ RL (RL Defense Agent):** Phát triển và huấn luyện tác tử phòng thủ. Trước khi cố định thuật toán, nhóm thực hiện bước sàng lọc trên các bộ dữ liệu CIC với ba thuật toán PPO/DQN/A2C theo cùng quy trình đánh giá chung để đảm bảo so sánh công bằng. Quy trình đánh giá (benchmark) này sử dụng siêu tham số mặc định nghiêm ngặt của thư viện SB3 (không tinh chỉnh bổ sung), `n_envs=1`, mô hình ở bước cuối cùng (final checkpoint), 5 hạt giống huấn luyện ngẫu nhiên (train seeds) và đánh giá tất định với 5 hạt giống đánh giá (eval seeds) × 6 tập (episodes) cho mỗi hạt giống; 4 chế độ đánh giá tách biệt hai trục phiên kết nối (session) và nhiễu (noise). Kết quả benchmark được trình bày ở Chương 4.

PPO được chọn so với các thuật toán RL khác vì ba lý do thực tiễn:

1. **Ổn định huấn luyện:** Cơ chế cắt gradient giúp tránh cập nhật chính sách quá lớn.
...
| learning_rate | — | 1×10⁻⁴ | ✓ Tốt | Giảm tuyến tính hoạt động đúng thiết kế |
...
**Tổng cộng ≈ 1.016s (1016ms).** Dù trễ hơn rule engine như Snort (< 1ms), bù lại agent phân tích hành vi tích lũy thay vì từng gói tin lẻ, dẫn đến FPR thấp hơn đáng kể và khả năng thích ứng với tấn công mới.
...
3. **SQLi Tránh né (sqlmap):** Phát hiện mẫu UNION SELECT — F14 = 1, F13 tăng mạnh. → **Hành động 2 (Chuyển hướng)** về cổng 4443. Kẻ tấn công tiếp tục cào CSDL trên Honeypot trong khi 192.168.10.10 hoàn toàn an toàn. Khi cửa sổ mềm 15 bước tích lũy đủ bằng chứng, bộ theo dõi IP thăng cấp → **Chặn**.

4. **XSS Botnet:** Tiêm `onerror=` — F18 tăng, F20 = 1. → **Hành động 2 (Chuyển hướng)** → thăng cấp Chặn khi cửa sổ mềm 15 bước đủ bằng chứng.

### 4.4.3 Phát hiện Kỹ thuật Chính

Ba quyết định thiết kế được xác nhận là then chốt cho hiệu suất hệ thống:

**Nhóm Đặc trưng HTTP (F12–F20):** Là điều kiện cần để nhận diện SQLi/XSS, vì các tấn công này không để lại dấu vết rõ ở tầng mạng. Việc tích hợp OWASP CRS 942 và 941 trực tiếp vào vector đặc trưng (F13, F18) giúp kế thừa tri thức chuyên gia bảo mật mà không phải xây dựng luật L7 từ đầu.

**Chuẩn hóa payload:** URL decode đệ quy (tối đa 2 vòng) xử lý kỹ thuật vượt qua mã hóa kép — kỹ thuật tránh né phổ biến nhất theo Spett [9]. Không có chuẩn hóa, payload như `%27%20OR%20%271%27%3D%271` sẽ xuất hiện như văn bản không thể nhận dạng với so khớp mẫu.

**Cửa sổ Trượt 1 Giây:** Điểm cân bằng giữa độ phân giải thời gian và ổn định thống kê. Cửa sổ dài làm mờ ranh giới giữa các pha tấn công, khiến agent khó phân biệt giai đoạn chuyển đổi.
...
- User cảnh báo được chuyển hướng sang honeypot (cửa sổ mềm 15 bước để xác minh hoặc escal) — không phải chặn ngay lập tức
...
Phát hiện **77,3%** được đưa ra dưới **một bộ trọng số cụ thể** của action cost/bonus và damage penalty — nó không phải "mức phát hiện tối ưu" mà là "mức phát hiện được học dưới design choice này."

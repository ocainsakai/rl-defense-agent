...
class MockIPBehavior:
    ...
    def __init__(self, ip_type: str, rng: np.random.Generator = None, missing_prob: float = 0.0):
        self.ip_type = ip_type
        self.rng = rng if rng is not None else np.random.default_rng()
        self.missing_prob = missing_prob  # Probability of zeroing each feature (except F1)
        self.step_counter = 0
    ...
    def get_features(self, enable_phase2: bool = True, missing_prob: float = None) -> List[float]:
        """Return current raw feature vector (20D list, FEATURE_CODES order).

        Args:
            enable_phase2: unused (kept for API compat)
            missing_prob: overrides self.missing_prob if provided
        ...
        # Feature dropout: simulate sensor noise / decryption lag / packet loss
        # Skip F1 (index 0) — keep PacketRate always visible for zone detection
        mp = missing_prob if missing_prob is not None else self.missing_prob
        if mp > 0.0:
            for i in range(1, 20):

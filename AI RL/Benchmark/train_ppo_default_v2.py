"""
PPO Default Baseline Training Script — v2 (30D Temporal Observation)

Train on standard env (env_ids.py, 30D obs) for fair comparison with PPO-Tuned v2.
SB3 recommended defaults — no tuning (same hyperparams as train_ppo_default.py).

Usage:
  python3 train_ppo_default_v2.py --seed 42
  for seed in 42 123 456 789 1337; do python3 train_ppo_default_v2.py --seed $seed; done
"""
...
def main():
    ...
    print(f"[*] Training PPO-Default-v2 (30D) | seed={seed} | {TOTAL_TIMESTEPS} steps")
    ...
    # SB3 PPO defaults — no tuning, wider first layer for 30D input
    model = PPO(
        "MlpPolicy",
        env,
